...
Dx_data <- 
  ...
  mutate(e65_cat = cut(e65, 
         ...
         PD_cat = cut(PD, 
                      ...
                      labels = 1:3, 
                      include.lowest = TRUE), 
         kreis_t_pos = ifelse(kreis_t_pos %in% c(3,4), 4, kreis_t_pos))
...
boot_mx_kr_sett <- function(x){
  ...
  ru <- x %>% filter(kreis_t_pos == 4)
  ur <- x %>% filter(kreis_t_pos == 1)
...
# dev.off()
pdf("figures/bar_plot_mortDiff_location.pdf", family = "Times", 
