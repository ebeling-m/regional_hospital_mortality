...
h_adj <- 
  ...
  select(Region, Age, sex, cfat, inc, kreis_t_pos) %>% 
...
# dev.off()
pdf("figures/MI_Median_incidence_CaseFatality_location.pdf", family = "Times", 
...
# Women
rur <- inc_median %>% filter(sex == "w", kreis_t_pos == 4)
...
urb <- inc_median %>% filter(sex == "w", kreis_t_pos == 1)
xx <- c(seq(0.5, 5.5, by=1), rev(seq(0.5, 5.5, by=1)))
...
rur <- inc_median %>% filter(sex == "m", kreis_t_pos == 4)
xx <- c(seq(0.5, 5.5, by=1), rev(seq(0.5, 5.5, by=1)))
yy <- c(rur$inc_1, rev(rur$inc_9))
polygon(x=xx, y=yy, col = adjustcolor('#5ab4ac', alpha.f = 0.2),
        border = "white")

urb <- inc_median %>% filter(sex == "m", kreis_t_pos == 1)
xx <- c(seq(0.5, 5.5, by=1), rev(seq(0.5, 5.5, by=1)))
yy <- c(urb$inc_1, rev(urb$inc_9))
polygon(x=xx, y=yy, col = adjustcolor('#d8b365', alpha.f = 0.2),
        border = "white")
