...
h_adj <- 
  ...
  select(Region, Age, sex, C_hosp_MI, Dx_MI_hosp_prim, Dx_MI, Dx_hosp_MI_sec, 
         Dx_CVD, Dx_tot, Nx, kreis_t_pos, cfat, inc, inc_inHosp, inc_outHosp)
...
median_est_urban <- 
  median_est %>% filter(kreis_t_pos == 1) %>% select(!kreis_t_pos)
...
h_adj_scenario <- 
  ...
  rowwise() %>% 
  mutate(inc_inHosp_A = 
           ifelse(inH_5 < inc_inHosp & kreis_t_pos == 4, 
                  inH_5, inc_inHosp),
         inc_outHosp_A = 
           ifelse(outH_5 < inc_outHosp & kreis_t_pos == 4, 
                  outH_5, inc_outHosp),
         cfat_A = 
           ifelse(cfat_5 < cfat & kreis_t_pos == 4, 
                  cfat_5, cfat), 
         inc_A =
           ifelse(inc_5 < inc & kreis_t_pos == 4, 
                  inc_5, inc)) %>% 
  mutate(Dx_MI_related = (Dx_MI-Dx_MI_hosp_prim+Dx_hosp_MI_sec),
         Dx_inH = (Nx*inc_inHosp_A + Nx*inc_outHosp)*cfat,
         Dx_outH =(Nx*inc_inHosp_A + Nx*inc_outHosp_A)*cfat,
...
#   mx_scenario %>% 
#   filter(Age == 65 & sex == "w") %>% 
#   mutate(col_p = ifelse(kreis_t_pos == 1, "black", "blue"))
# 
# test2 <- 
#   median_mx_scenario %>% 
#   filter(Age == 65 & sex == "w")
...
Dx_scenario_tot <- 
  mx_scenario %>% 
  group_by(sex, kreis_t_pos) %>% 
  summarise(across(c(Dx_MI_related,Dx_inH:Dx_full), ~ sum(.x)))

test <- Dx_scenario_tot %>% filter(kreis_t_pos == 4)
(sum(test$Dx_MI_related)-sum(test$Dx_full))/sum(test$Dx_MI_related)
...
basic_bar_counts <- function(){
  plot(x=1, y=1, typ = "n", lwd = 3, xlim = c(0, 14000), ylim = c(0,6), 
       bty = "n", xaxt = "n", yaxt = "n", xaxs = "i", yaxs = "i", 
       xlab = NA, ylab = NA)
  segments(x0=seq(0, 14000, by = 2000), y0=0,
           x1=seq(0, 14000, by = 2000), y1=6, 
           col = "lightgray")
  
  axis(1, at = seq(0, 14000, by = 2000), 
       labels = TRUE, lwd = 3, cex.axis = 1.4, pos = -0.08)
  axis(1, at = seq(1000, 13000, by = 2000), 
...
pdf("figures/counterfactual_location.pdf", family = "Times",
...
# Women
basic_bar_counts()
wom <- Dx_scenario %>% filter(sex == "w" & kreis_t_pos == 4)
...
# Men
basic_bar_counts()
wom <- Dx_scenario %>% filter(sex == "m" & kreis_t_pos == 4)
yy <- cbind(seq(0.1, 5.1, by = 1),
            seq(0.4, 5.4, by = 1),
            seq(0.6, 5.6, by = 1),
            seq(0.8, 5.8, by = 1))
