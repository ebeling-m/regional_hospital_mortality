...
hosp_merge <- 
  hosp %>% 
  mutate(kreis_t_pos = ifelse(kreis_t_pos %in% c(3,4), 4, kreis_t_pos)) %>%
  select(Region, sex, Age, C_hosp_MI, Dx_MI, 
         Dx_MI_hosp_prim, Nx, kreis_t_pos)
...
comp_MI_rate <-
  ...
  group_by(Region, Age, sex, Nx, aufn_anl, kreis_t_pos, 
           Dx_MI_hosp_prim, Dx_MI) %>%
  summarise(C_hosp_MI_adm = sum(C_hosp_MI_adm)) %>%
  group_by(Region, Age, sex) %>% 
  ungroup() %>%
  mutate(out_hosp = Dx_MI-Dx_MI_hosp_prim) %>% 
  select(Region, Age, sex, kreis_t_pos, Nx, aufn_anl, C_hosp_MI_adm)
...
out_hosp <- 
  ...
  ungroup() %>%
  mutate(out_hosp = Dx_MI-Dx_MI_hosp_prim) %>% 
  distinct(Region, Age, sex, kreis_t_pos, Nx, out_hosp) %>% 
  mutate(aufn_anl = "D") %>% 
...
comp_MI <- 
  ...
  group_by(Region, Age, sex, aufn_anl, kreis_t_pos, Nx) %>%
  summarise(C_hosp_MI_adm = sum(C_hosp_MI_adm)) %>% 
  filter(kreis_t_pos %in% c(1,4)) %>%
  ungroup() %>% 
  ...
  group_by(Age, sex, kreis_t_pos, aufn_anl) %>% 
...
pdf("figures/in_out_hospital_eventRates_location.pdf", family = "Times", 
...
rur_X <- 
  comp_MI %>% 
  filter(sex == "w", kreis_t_pos == 4, aufn_anl == "X")
rur_D <- 
  comp_MI %>% 
  filter(sex == "w", kreis_t_pos == 4, aufn_anl == "D")

urb_X <- 
  comp_MI %>% 
  filter(sex == "w", kreis_t_pos == 1, aufn_anl == "X")
urb_D <- 
  comp_MI %>% 
  filter(sex == "w", kreis_t_pos == 1, aufn_anl == "D")
...
rur_X <- 
  comp_MI %>% 
  filter(sex == "m", kreis_t_pos == 4, aufn_anl == "X")
rur_D <- 
  comp_MI %>% 
  filter(sex == "m", kreis_t_pos == 4, aufn_anl == "D")

urb_X <- 
  comp_MI %>% 
  filter(sex == "m", kreis_t_pos == 1, aufn_anl == "X")
urb_D <- 
  comp_MI %>% 
  filter(sex == "m", kreis_t_pos == 1, aufn_anl == "D")
...
comp_MI_events <-
  h_case_adj_adm %>%
  group_by(kreis_t_pos, Age, sex, aufn_anl) %>%
  summarise(C_hosp_MI_adm = sum(C_hosp_MI_adm),
            Dx_MI = sum(Dx_MI), 
            Dx_MI_hosp_prim = sum(Dx_MI_hosp_prim)) %>%
  group_by(kreis_t_pos, Age, sex) %>%
  mutate(C_hosp_MI_SumAdm = sum(C_hosp_MI_adm)) %>% 
  ungroup() %>%
  mutate(MI_events = C_hosp_MI_SumAdm+(Dx_MI-Dx_MI_hosp_prim),
         out_hosp = Dx_MI-Dx_MI_hosp_prim,
         case_share = C_hosp_MI_adm / MI_events) %>%
  select(Age, sex, kreis_t_pos, case_share, aufn_anl) %>% 
  filter(kreis_t_pos %in% c(1,4)) %>% 
  arrange(sex, Age, aufn_anl) %>% 
  group_by(Age, sex, kreis_t_pos) %>% 
  mutate(cu_share = cumsum(case_share))

col_rur <- adjustcolor(c('#edf8e9','#bae4b3','#74c476','#238b45'),
                       alpha.f = 0.7)
col_urb <- adjustcolor(c('#eff3ff','#bdd7e7','#6baed6','#2171b5'),
                       alpha.f = 0.7)

pdf("figures/distribution_MIevents_location.pdf", family = "Times", 
...
legend("center", legend =c("inter-hospital transfer", "emergency admission",
                           "other admission", "death outside hospital"), 
       col = c('#f7f7f7','#cccccc','#969696','#525252'), 
       bty = "n", cex = 1.3, pch = 15, horiz = TRUE)
dev.off()
